...
export default function ProfilePageClient() {
  ...
  return (
    <main className='container !my-10'>
      {/* ===== Hero / Header ===== */}
      <section className="relative rounded-3xl overflow-hidden border border-main-200 bg-white dark:bg-dark-bg-card shadow-lg">
        {/* Gradient overlay */}
        <div className="absolute inset-0 bg-gradient-to-r from-main-500 to-main-400 opacity-95 dark:opacity-40 backdrop-blur-sm" />

        <div className="relative p-6 sm:p-8 text-white rounded-2xl">
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-5 rounded-2xl">
            {/* Avatar */}
            <div className="h-20 w-20 grid place-items-center rounded-2xl bg-white/15 dark:bg-white/10 ring-4 ring-white/10 text-white text-3xl font-extrabold shadow">
              {initials}
            </div>
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center flex-wrap gap-3">
                <h1 className="text-2xl sm:text-3xl font-semibold tracking-tight truncate drop-shadow">
                  {toTitle(name)}
                </h1>
                ...
                <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold ring-1 ring-white/30 bg-white/15 dark:bg-white/10">
              ...
              <div className="mt-2 text-sm/6 flex flex-wrap items-center gap-x-4 gap-y-1 text-white/90">
                ...
                <span className="inline-flex items-center gap-1.5">
                  <Clock className="h-4 w-4" /> {t('lastLogin')} {prettyDate(buyer?.lastLogin)}{' '}
                  <em className="text-white/70">({fromNow(buyer?.lastLogin)})</em>
                </span>
              </div>
            </div>
    ...
    bg-white dark:bg-white/80 text-main-700
    hover:bg-main-50 dark:hover:bg-white/60
    focus:outline-none focus-visible:ring-2 focus-visible:ring-main-500
    active:scale-95 transition shadow-lg sm:min-w-[140px]
  "
            >
          ...
          <div className="mt-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
            ...
            <StatCard
              ...
              icon={Star}
              title={t('orders')}
              value={buyer?.ordersCompleted ?? 0}
            />
            <StatCard
              gradient="from-amber-400 via-orange-500 to-rose-500"
              icon={Repeat}
              title={t('repeatBuyers')}
              value={buyer?.repeatBuyers ?? 0}
            />
            <StatCard
      ...
      <section className="mt-6 grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1">
          <div className="sticky top-30 space-y-6">
            <Card title={t('about')}>
              <p className="text-slate-700 dark:text-dark-text-secondary leading-relaxed">
                {buyer?.description || t('noDescription')}
              </p>
                ...
                {Array.isArray(buyer?.education) && buyer.education.length > 0 ? (
                  <div className="space-y-3">
                    {buyer.education.map((ed, idx) => (
                      <div
                        key={idx}
                        className="rounded-xl border border-slate-200 dark:border-dark-border bg-slate-50/60 dark:bg-dark-bg-card p-4"
                      >
                        <div className="flex items-start gap-3">
                          <GraduationCap className="h-5 w-5 text-main-500 shrink-0 mt-0.5" />
                          <div className="min-w-0">
                            <div className="font-semibold text-slate-900 dark:text-dark-text-primary">{ed?.degree || '—'}</div>
                            <div className="text-sm text-slate-600 dark:text-dark-text-secondary">{ed?.institution || '—'}</div>
                            {ed?.year && (
                  ...
                  <div className="rounded-xl border border-dashed border-slate-300 dark:border-dark-border bg-slate-50/60 dark:bg-dark-bg-card/30 p-4 text-slate-500 dark:text-dark-text-secondary text-sm">
                    {t('noEducation')}
                  </div>
                )}
              </Card>
                    ...
                    {buyer.certifications.map((cert, idx) => (
                      ...
                      >
                        <div className="flex items-start gap-3">
                          ...
                          <div className="min-w-0">
                            <div className="font-semibold text-slate-900 dark:text-dark-text-primary">{cert?.name || '—'}</div>
                            <div className="text-sm text-slate-600 dark:text-dark-text-secondary">{cert?.issuingOrganization || '—'}</div>
                            {cert?.year && (
                              <div className="text-xs text-slate-500 dark:text-dark-text-secondary mt-1">
                                {t('year')}: {cert.year}
                              </div>
            ...
            <Card title={t('rates')}>
              ...
              {nextCursor && (
                <div className="pt-4 text-center">
                  <Button
                    type="button" // keep as button, not submit
                    name={loadingMore ? t('loading') : t('loadMore')}
                    color="green"
                    className='w-fit!'
                    disabled={loadingMore} // preserve the loading state
  ...
  return (
    <main className="container !my-10">
      <div className="mx-auto px-4 sm:px-6 lg:px-8 animate-pulse">
        <div className="h-36 rounded-3xl bg-slate-100 dark:bg-dark-bg-card" />
        <div className="mt-6 grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[...Array(4)].map((_, i) => (
            <div key={i} className="h-20 rounded-xl bg-slate-100 dark:bg-dark-bg-card" />
          ))}
        </div>
        <div className="mt-6 grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="h-64 rounded-2xl bg-slate-100 dark:bg-dark-bg-card" />
          <div className="lg:col-span-2 h-64 rounded-2xl bg-slate-100 dark:bg-dark-bg-card" />
        </div>
      </div>
    </main>
  );
  ...
  return (
    <div className="space-y-4">
      {[1, 2, 3].map((i) => (
        <div
          key={i}
          className="p-5 rounded-2xl border border-slate-200 dark:border-dark-border bg-white dark:bg-dark-bg-card shadow-sm animate-pulse"
        >
          <div className="flex justify-between items-start mb-4">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-full bg-slate-200 dark:bg-dark-bg-input" />
              <div className="space-y-2">
                <div className="h-3 w-24 bg-slate-200 dark:bg-dark-bg-input rounded" />
                <div className="h-2 w-16 bg-slate-100 dark:bg-dark-bg-card rounded" />
              </div>
            </div>
            <div className="h-4 w-20 bg-slate-200 dark:bg-dark-bg-input rounded" />
          </div>
          <div className="space-y-2">
            <div className="h-3 w-full bg-slate-100 dark:bg-dark-bg-card rounded" />
            <div className="h-3 w-5/6 bg-slate-100 dark:bg-dark-bg-card rounded" />
          </div>
        </div>
      ))}
    </div>
...
function Card({ title, children }) {
  return (
    <div className="rounded-2xl border border-slate-200 dark:border-dark-border bg-white dark:bg-dark-bg-card p-4 sm:p-5 shadow-sm">
      {title && (
        <h2 className="text-sm font-semibold text-slate-900 dark:text-dark-text-primary tracking-wide mb-3">
          {title}
        </h2>
      )}
      {children}
  ...
  return (
    <div className="flex items-center justify-between gap-3 py-2">
      <div className="flex items-center gap-2 min-w-0">
        {Icon && <Icon className="h-4 w-4 text-slate-500 dark:text-dark-text-secondary" />}
        <span className="text-sm text-slate-500 dark:text-dark-text-secondary">{label}</span>
      </div>
      <div className="flex items-center gap-2 min-w-0">
        <span
          className="text-sm font-medium text-slate-900 dark:text-dark-text-primary truncate max-w-[280px]"
          title={String(val)}
        >
          {String(val)}
        </span>
        {copyable && (
          <button
            ...
            className="inline-flex items-center justify-center rounded-md border border-slate-200 dark:border-dark-border px-2 py-1 text-xs text-slate-700 dark:text-dark-text-primary hover:bg-slate-50 dark:hover:bg-dark-bg-input active:scale-95"
          >
            {t('copy')}
          </button>
        )}
      ...
      <div className="flex items-center gap-2 min-w-0">
        ...
        {value && (
          <div className="flex items-center gap-2">
            <span
              className="text-sm font-medium text-slate-900 dark:text-dark-text-primary truncate max-w-[150px]"
              title={String(value)}
            >
              {String(value).trim()}
            </span>
            {copyable && (
              <button
                ...
                className="inline-flex items-center justify-center rounded-md border border-slate-200 dark:border-dark-border px-2 py-0.5 text-[10px] text-slate-700 dark:text-dark-text-primary hover:bg-slate-50 dark:hover:bg-dark-bg-input active:scale-95"
              >
                {tc('copy')}
              </button>
            )}
        ...
        {verified ? (
          <div className="flex items-center gap-1.5 text-main-600 dark:text-main-300">
...
function Badge({ icon: Icon, text }) {
  return (
    <span className="inline-flex items-center gap-1.5 rounded-full bg-main-50 text-main-700 ring-1 ring-main-200 px-2.5 py-1 text-xs font-semibold dark:bg-main-900 dark:text-main-300 dark:ring-main-700">
...
function ServiceCard({ service }) {
  ...
  return (
    <div className="group rounded-2xl border border-slate-200 dark:border-dark-border bg-white dark:bg-dark-bg-card p-4 shadow-sm hover:shadow-md transition-all duration-300">
      {/* Top row */}
      <div className="flex items-center gap-3">
        <div className="flex-none h-11 w-11 grid place-items-center rounded-xl bg-gradient-to-r from-main-500 to-main-400 text-white font-semibold shadow">
          {initials}
        </div>
        <div className="min-w-0 truncate whitespace-nowrap">
          <Link
            href={`/services/category/${service?.slug || service?.id || ''}`}
            className="font-semibold text-slate-900 dark:text-dark-text-primary group-hover:text-main-600 dark:group-hover:text-main-400"
            title={service?.title || t('untitledService')}
          >
            {service?.title || t('untitledService')}
          </Link>
          {service?.category && (
            <div className="mt-0.5 text-xs text-slate-500 dark:text-dark-text-secondary truncate">
              {service.category}
            </div>
          )}
        </div>
...
function JobCard({ job }) {
  ...
  const budgetLabel =
    job?.budgetType === 'hourly' ? `${formatMoney(budget)}/hr` : formatMoney(budget);
  ...
  return (
    <div className="group rounded-2xl border border-slate-200 dark:border-dark-border bg-white dark:bg-dark-bg-card p-4 shadow-sm hover:shadow-md transition-all duration-300">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0 truncate whitespace-nowrap">
          <Link
            ...
            className="font-semibold text-slate-900 dark:text-dark-text-primary group-hover:text-indigo-600 dark:group-hover:text-main-400"
      ...
      {/* Description */}
      <p className="mt-3 text-sm text-slate-700 dark:text-dark-text-secondary whitespace-pre-wrap line-clamp-4">
        {job?.description}
      </p>
    </div>
  );
...
function ReviewItem({ review, profileId }) {
  ...
  return (
    <div className="p-5 rounded-2xl border border-slate-100 dark:border-dark-border bg-white dark:bg-dark-bg-card shadow-sm transition-all hover:border-indigo-100 dark:hover:border-main-400">
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-center gap-3">
          {/* Reviewer Image */}
          <div className="h-10 w-10 shrink-0 overflow-hidden rounded-full border border-slate-100 dark:border-dark-border bg-slate-50 dark:bg-dark-bg-input">
            <Img
        ...
        <div className="flex flex-col items-end gap-1">
          ...
          <span className="text-xs font-bold text-slate-700 dark:text-dark-text-primary">
            {score?.toFixed(1)}
          </span>
        </div>
      </div>
      ...
      <div className="mt-4 text-sm leading-relaxed">
        {hasText ? (
          <>
            ...
            {shouldTruncate && (
              <button
                ...
                className="mt-2 flex items-center gap-1 text-xs font-bold text-indigo-600 dark:text-main-400 hover:underline"
              >
                {isExpanded ? (
                  <>
          ...
          <p className="text-slate-400 dark:text-dark-text-secondary italic">
            {t('noTextFeedback')}
          </p>
        )}
      </div>
...
function Metric({ icon: Icon, label, value }) {
  return (
    <div className="rounded-lg border border-slate-200 dark:border-dark-border bg-slate-50/60 dark:bg-dark-bg-card/60 px-2.5 py-2">
      <div className="flex items-center gap-1.5 text-slate-600 dark:text-dark-text-secondary text-xs">
        <Icon className="h-3.5 w-3.5 text-main-500" />
        <span>{label}</span>
      </div>
      <div className="mt-0.5 text-sm font-semibold text-slate-900 dark:text-dark-text-primary">
        {value}
      </div>
    </div>
  );
...
function EmptyState({ text, actionHref, actionText }) {
  return (
    <div className="rounded-xl border border-dashed border-slate-300 dark:border-dark-border p-8 text-center bg-slate-50/60 dark:bg-dark-bg-card/60">
      <div className="text-slate-600 dark:text-dark-text-secondary text-sm">{text}</div>
      {actionHref && (
        <Link
          href={actionHref}
          className="mt-4 inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-gradient-to-r from-main-500 to-main-400 text-white text-sm font-semibold shadow-lg transition-all duration-300 hover:brightness-110 active:scale-95"
        >
