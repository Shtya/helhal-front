...
export const SelectInput = ({
  ...
  return (
    <div className="w-full mb-5">
      <div className="p-4 border border-slate-200 dark:border-dark-border rounded-xl shadow-inner dark:bg-dark-bg-card transition-colors duration-300">
        {label && (
              ...
              return (
                <motion.button
                  ...
                  className={`relative rounded-2xl px-6 py-2 transition-colors duration-200
                    ${isActive
                      ? 'bg-main-500 dark:bg-main-500 text-white'
                      : 'bg-slate-100 dark:bg-dark-bg-input text-gray-700 dark:text-dark-text-primary'}
                    ${disabled ? 'cursor-not-allowed opacity-50' : 'cursor-pointer'}
                    ${isHovered && !disabled ? 'bg-main-600 dark:bg-main-600' : ''}`}
                  disabled={disabled}
                >
                  {isActive && (
                    <motion.span
