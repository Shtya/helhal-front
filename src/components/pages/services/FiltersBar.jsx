...
export default function FiltersBar({ onChange }) {
  ...
  const box = 'h-12 rounded-xl border border-emerald-600/70 focus-within:ring-2 focus-within:ring-emerald-500/80 px-4 bg-transparent';

  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 my-6">
      <input
        placeholder="Service Options"
        className={`${box} text-sm placeholder:text-emerald-400/70`}
        value={budget}
      ...
      <div className="flex items-center gap-2">
        <input
          ...
          className={`flex-1 ${box} text-sm placeholder:text-emerald-400/70`}
          value={location}
          onChange={(e) => setLocation(e.target.value)}
        />
        <select
          className="h-12 rounded-xl border border-emerald-600/70 bg-transparent px-3 text-sm"
          value={sort}
