...
export default function AmazingServiceCard({ isHoverScale = true, service, href, index = 0 }) {
  ...
  const cardVariants = {
    ...
    hover: { y: -6, rotateX: 2, rotateY: -2, transition: { type: 'spring', stiffness: 220, damping: 18 } },
  };

  return (
    <motion.article className={` ${isHoverScale ? 'hover:scale-[1.05]' : 'hover:scale-[1.01] '} relative pb-[60px] h-full group service-card !rounded-[20px] !overflow-hidden bg-white text-black border border-slate-200 shadow-sm transition-all duration-300 will-change-transform`} variants={cardVariants} initial='initial' animate='in' role='article'>
      {/* Cover */}
      <div className='relative group'>
        <div className='relative w-full aspect-[16/10]  overflow-hidden rounded-[20px_20px_0_0]'>
          ...
          {/* Hover overlay – slides in from right; no scroll; clips extra */}
          <div className=' absolute inset-0 text-white p-4 flex flex-col justify-end backdrop-blur-xs bg-gradient-to-t from-black/70 via-black/30 to-transparent translate-y-full opacity-0 transition-all duration-400 ease-out group-hover:translate-y-0 group-hover:opacity-100 overflow-hidden '>
            {/* Feature bullets limited to 4, clipped after */}
      ...
      <div className='p-4 space-y-3  '>
        <Link href={to} className='block focus:outline-none focus:ring-2 focus:ring-blue-500 rounded'>
          <h3 className='text-[17px] font-semibold line-clamp-2'>{serviceTitle}</h3>
        </Link>
