...
const TabsPagination = ({ loading, recordsCount, currentPage, totalPages, onPageChange, onItemsPerPageChange, itemsPerPage, className,
  options = [
    { id: 5, name: '5' },
    { id: 10, name: '10' },
    { id: 20, name: '20' },
      ...
      <div className={`flex flex-col xs:flex-row items-center xs:justify-between w-full gap-4 mt-6 ${loading ? 'opacity-60 pointer-events-none' : ''}` + (className ? ` ${className}` : '')}>
        ...
        <div className={`flex items-center space-x-1 xs:space-x-2  ${onItemsPerPageChange ? 'justify-center xs:justify-start' : 'flex-1 justify-between'}`}>
          <button
            ...
            className={`px-4 py-2 cursor-pointer gradient text-white ${isRTL ? "rounded-r-md" : "rounded-l-md"} disabled:!opacity-30`}
          >
            {t('prev')}
          </button>
          ...
          <button
            ...
            className={`px-4 py-2 cursor-pointer gradient text-white ${isRTL ? "rounded-l-md" : "rounded-r-md"} disabled:opacity-30`}
          >
            {t('next')}
          </button>
        </div>
