  ...
  return (
    <div className=' mb-4 '>
      <div className='flex items-center gap-3 sm:gap-4'>
        {/* Letter avatar only */}
        <div className='h-10 w-10 sm:h-10 sm:w-10 grid place-items-center rounded-full gradient text-white font-semibold'>{initials}</div>

        <div className='flex flex-col justify-center '>
          <Link href={profileHref} className='flex items-center gap-2 font-semibold text-slate-900 hover:text-indigo-600 truncate' title={name}>
            {toTitle(name)} 
						<span className={` inline-flex items-center text-[11px] gap-1 px-2 py-0.5 rounded-full border ${roleTone(role)}`}>{role}</span>
          </Link>
          <div className='text-xs mt-1 text-slate-500 truncate' title={email}>
            {email}
          </div>
        </div>
      </div>
