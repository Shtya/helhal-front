...
export function PermissionRow({ domain, value = 0, onChange, removeAll }) {
    ...
    return (
        <div className="flex max-md:gap-4 md:items-center flex-col md:flex-row bg-white dark:bg-dark-bg-card p-4 rounded-lg shadow-sm border border-gray-100 dark:border-dark-border mb-3">
            {/* Domain Label */}
            <div className="w-1/3 min-w-[150px] font-bold text-gray-800 dark:text-dark-text-primary text-lg px-2">
                {t(`domains.${domain.labelKey}`)}
            </div>

            {/* Vertical Divider */}
            <div className="max-md:hidden h-10 w-px bg-gray-200 dark:bg-dark-border mx-4" />
                ...
                {domain.actions.map(action => (
                    <label key={action.value} className="flex flex-row items-center gap-2 cursor-pointer group">
                        ...
                        <input
                            ...
                            className="w-3.5 h-3.5 rounded border-gray-300 dark:border-dark-border text-blue-600 focus:ring-blue-500 cursor-pointer"
                        />
                        {/* Action Label */}
                        <span className="text-[10px] capitalize font-semibold text-gray-700 dark:text-dark-text-secondary group-hover:text-indigo-500 transition-colors">
